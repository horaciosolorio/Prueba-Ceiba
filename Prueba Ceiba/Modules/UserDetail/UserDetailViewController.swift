...
class UserDetailViewController: UIViewController, UserDetailViewProtocol {
    ...
    func showEmptyResults() {
        let alert = UIAlertController(
            title: "Aviso", message: "No hay resultados intenta con un radio mas amplio", preferredStyle: .alert
        )
        let done = UIAlertAction(title: "Entendido", style: .destructive, handler: { (_) -> Void in })
...
extension UserDetailViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(
            withIdentifier: cellReuseIdentifier, for: indexPath) as? PostTableViewCell else {
